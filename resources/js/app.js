...
import HomepageImageHandler from './homepage-image-handler';
...
const SwalAlert = {
    ...
    /**
     * Loading modal (same chrome as error/success/info)
     */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.productsHandler = new ProductsHandler({
    ...
    window.usersHandler = new UsersHandler({
    ...
    // Initialize homepage image drop zones on pages edit (home, product, etc.)
    if (document.querySelector('.homepage-image-upload') || document.querySelector('.homepage-partners-upload')) {
        window.homepageImageHandler = new HomepageImageHandler({
            maxSize: 10 * 1024 * 1024,
            acceptedTypes: ['image/jpeg', 'image/png', 'image/gif', 'image/webp']
        });
        window.homepageImageHandler.init();
    }

    // Initialize Categories Handler on category create/edit pages
    window.categoriesHandler = new CategoriesHandler();

    // Initialize Page Handler on page create/edit pages
    window.pagesHandler = new PagesHandler();

    // Initialize Roles Handler on role create/edit pages
    window.rolesHandler = new RolesHandler();
});
    ...
    document.querySelectorAll('.datetime[data-iso]').forEach(function(el) {
        ...
        if (iso && !el.dataset.converted) {
            ...
            if (!isNaN(d.getTime())) {
                ...
                el.textContent = includeSeconds ? (date + ' ' + time) : (date + ' at ' + time);
...
/**
 * Copy JSON/value to clipboard (site setting show page).
 * @param {Event} event - Click event (target should be inside .copy-json-btn or the button)
 */
