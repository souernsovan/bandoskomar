                ...
                <div class="form-group">
                    ...
                    <select id="group" name="group" class="form-input @error('group') error @enderror" required>
                        ...
                        <option value="analytics"
                            {{ old('group', $siteSetting->group) === 'analytics' ? 'selected' : '' }}>
                            Analytics</option>
            ...
            <div class="form-grid">
                <div class="form-group full-width">
                    <label class="form-label">Value
                        @if (!in_array($siteSetting->key, ['available_avatars', 'site_logo', 'site_icon'], true))
                    ...
                    @elseif($siteSetting->key === 'site_logo')
                        @php
                            $logoPath = is_string($siteSetting->value)
                                ? $siteSetting->value
                                : \App\Models\SiteSetting::DEFAULT_SITE_LOGO;
                        @endphp
                        <div class="site-branding-upload">
                            <p class="form-hint" style="margin-bottom: 1rem;">Used in the admin sidebar, login page, and
                                frontend header. PNG, JPG, GIF, WebP, or SVG. Max 2&nbsp;MB.</p>
                            <div class="avatar-image-container" style="max-width: 280px; margin-bottom: 1rem;">
                                <img src="{{ asset($logoPath) }}" alt="Current site logo" class=""
                                    style="object-fit: contain; max-height: 120px; width: 100%;">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="site_logo_file">Upload new logo</label>
                                <input type="file" id="site_logo_file" name="site_logo_file" class="form-input"
                                ...
                                <div class="form-group" style="margin-top: 1rem;">
                                    <label class="form-label"
                                        ...
                                        Reset to default logo (bundled file)
                                    </label>
                                </div>
                                <input type="hidden" name="value" value="{{ old('value', $logoPath) }}">
                            </div>
                        </div>
                    @elseif($siteSetting->key === 'site_icon')
                        @php
                            $iconPath = is_string($siteSetting->value)
                                ? $siteSetting->value
                                : \App\Models\SiteSetting::DEFAULT_SITE_ICON;
                        @endphp
                        <div class="site-branding-upload">
                            <p class="form-hint" style="margin-bottom: 1rem;">Favicon / browser tab icon. PNG, ICO, SVG,
                                JPG, GIF, or WebP. Max 512&nbsp;KB.</p>
                            <div class="avatar-image-container" style="width: 64px; height: 64px; margin-bottom: 1rem;">
                                <img src="{{ asset($iconPath) }}" alt="Current site icon" class=""
                                    style="object-fit: contain; width: 64px; height: 64px;">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="site_icon_file">Upload new icon</label>
                                <input type="file" id="site_icon_file" name="site_icon_file" class="form-input"
                                    accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml,.ico,image/x-icon">
                                ...
                                <div class="form-group" style="margin-top: 1rem;">
                                    <label class="form-label"
                                        style="display: flex; align-items: center; gap: 0.5rem; font-weight: normal;">
                                        <input type="checkbox" name="remove_site_icon" value="1">
                                        Reset to default icon (bundled file)
                                    </label>
                                </div>
                                <input type="hidden" name="value" value="{{ old('value', $iconPath) }}">
                            </div>
                        </div>
                    @else
